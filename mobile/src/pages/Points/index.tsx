...
import { View, StyleSheet, Text, Image, InteractionManager, Alert } from 'react-native';
...
const Points = () => {
  ...
  const [points, setPoints] = useState<interfacePoints[]>([]);
  const [selectedItems, setSelectedItems] = useState<number[]>([]);
  const [initialPosition, setInitialPosition] = useState<[number,number]>([0,0]);

  const route = useRoute();

  const routeParams = route.params as interfaceParams;
  ...
  useEffect(() => {
    async function loadPosition() {
      ...
      if (status !== 'granted') {
        Alert.alert('Opps...', 'Precisamos de sua localização');
        return;
      }
      try {
  ...
  function handleNavigatorToDetail(id: number){
    navigation.navigate('Detail', {id_point: id});
  }
